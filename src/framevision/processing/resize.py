...
class Resize:
    def __init__(self, size: Optional[list[int]] = None, scale: Optional[float] = None):
        self.size = size
        self.scale = scale
    ...
    def adjust_intrinsics(self, intrinsics: dict[str, np.ndarray], scale_w: float, scale_h: float) -> dict[str, np.ndarray]:
        K = intrinsics["K"].copy()
